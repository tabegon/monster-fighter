...
def choix_hero():
        ...
        while True:
            ...
            if choix == "1":
                return  Hero('Guerrier', 10, 20, 30, 30, 5)
            if choix == "2":
...
def regeneration_vie_personage():
    global vies, joueur
    if is_action_valid:
        ...
        joueur = joueur + vies
        if joueur > Hero.reinitialiser_vie():
            personage.vie = reinitialiser_vie()
            joueur = personage.vie


def esquiver_attaque_personage():
...
def attaque_speciale_personage():
...
def code_boss_final():
    global boss, Attack_Boss, action, attack, coup_critique, vies, joueur, esquive, après, Attack_spéciale, coup_spécial, action_boss, attack_b, vies_b
    if boss_final == "oui":
        boss = 500
        Attack_Boss = 25
...
while boss > 0 and joueur > 0:
    ...
    if action_boss == 2:
        if action == 1:
            ...
            print(f'Ils vous reste {joueur} vies')
        if action == 2:
            print ("Le boss n'a pas réussi a vous esquivez.")

    if boss <= 0:
        boss = rénitialiser_boss + 10
        rénitialiser_boss = rénitialiser_boss + 10
        Attack_Boss = Attack_Boss + 5
        personage.reinitialiser_vie
        joueur = personage.vie + 2
        ...
        if Étages == 20:
            ...
            boss_final = "oui"
        print(f"\033[32mTu as gagné {emoji} maintenant tu est à l'étage n°{Étages}\033[0m")
